...
public class IoCApplicationContex {
	...
	public IoCApplicationContex(String file) throws Exception{
		...
		Document xmlDoc = sb.build(this.getClass().getClassLoader().getResourceAsStream(file));//构造文档对象
		...
		for(int i = 0;i<list.size();i++){
			...
			if(e.getChild("property")!=null){
				for(Element p : (List<Element>)e.getChildren("property")){
					...
					String bean = p.getAttributeValue("ref"); //引用类型 inf
					String value = p.getAttributeValue("value"); //简单类型 value
